...
describe('basic parse test', () => {
    ...
    it('should parseChars', () => {
        ...
        expect(parser.parseChars(`Harry P., Hermione G.`)).to.eql([
            [],
            ['Harry P.', 'Hermione G.'],
        ]);
        expect(parser.parseChars(`[Harry P., Hermione G.]`)).to.eql([
        ...
        expect(
            parser.parseChars(`[Harry P., Hermione G., Fleur D., N. Tonks]`),
        ).to.eql([[['Harry P.', 'Hermione G.', 'Fleur D.', 'N. Tonks']], []]);
        expect(
            parser.parseChars(`[Hermione G., Draco M.] Sirius B., Regulus B.`),
        ).to.eql([[['Hermione G.', 'Draco M.']], ['Sirius B.', 'Regulus B.']]);
        expect(
            parser.parseChars(
                `[Severus S., Sirius B.] [Lily Evans P., James P.]`,
            ),
        ).to.eql([
            [
                ['Severus S.', 'Sirius B.'],
    ...
    it('should throw exception on wrong format', () => {
        expect(() =>
            parser.parseChars(`Harry P., Hermione G. [Hermione G., Draco M.]`),
        ).to.throwException('Can`t parse characters');
    });
});
...
describe('parser parseSearchDivData', () => {
    it('should handle the basics', () => {
        expect(
            parser.parseSearchDivData(
                `<div class="z-indent z-padtop">Description<div class="z-padtop2 xgray">Har-ry Potter - Rated: K+ - English - Angst/Romance - Chapters: 4 - Words: 4,669 - Reviews: 24 - Favs: 4 - Follows: 3 - Updated: <span data-xutime="989132400">May 6, 2001</span> - Published: <span data-xutime="988527600">Apr 29, 2001</span> - Draco M., Harry P.</div></div>`,
            ),
        ).to.eql({
        ...
        expect(
            parser.parseSearchDivData(
                `<div class="z-indent z-padtop">Description<div class="z-padtop2 xgray">Rated: M - English - Romance/Angst - Chapters: 49 - Words: 284,050 - Reviews: 18615 - Favs: 37,499 - Follows: 19,755 - Updated: <span data-xutime="1578190357">Jan 5, 2020</span> - Published: <span data-xutime="1283431425">Sep 2, 2010</span> - Hermione G., Draco M. - Complete</div></div>`,
            ),
        ).to.eql({
            fandom: undefined,
            xfandom: null,
        ...
        expect(
            parser.parseSearchDivData(
                `<div class="z-indent z-padtop">Description<div class="z-padtop2 xgray">Rated: M - Spanish - Hurt/Comfort/Romance - Chapters: 1 - Words: 2,918 - Published: <span data-xutime="1678461818">1h ago</span> - [Harry P., OC] Voldemort</div></div>`,
            ),
        ).to.eql({
            fandom: undefined,
            xfandom: null,
    ...
    it('should handle crossovers', () => {
        expect(
            parser.parseSearchDivData(
                `<div class="z-indent z-padtop">At the end of the war, Alina travels the world.<div class="z-padtop2 xgray">Crossover - Shadow and Bone &amp; House of the Dragon - Rated: T - English - Adventure/Friendship - Chapters: 1 - Words: 1,709 - Published: <span data-xutime="1689099337">6h ago</span> - Alina S., Rhaenyra T. - Complete</div></div>`,
            ),
        ).to.eql({
    ...
    it('should handle strange fandom names', () => {
        ...
        expect(
            ...
            completed: false,
            description:
                'Rated T for swearing and perverted suggestions. Stocking ran out of sweets, and on her quest to find some, she nearly gets into a car accident thanks to a very special card. As if to guide her onto a particular path, a card shop is near where she parks, and a certain Geek Boy is running the shop...',
        });
    });
...
describe('parser parseSearchPage', () => {
    it('should handle the basics', () => {
        expect(
        ...
        ).to.eql([
            {
                ...
                completed: false,
                description:
                    "AU. Petunia Evans didn't marry Vernon Dursley – instead she went for the seer, Daniel Pasturl. So when Harry Potter turns up on their doorstep, how will the couple handle it? Will Harry get the childhood he deserved? Dumbledore bashing. Ron bashing. Intelligent!Harry Seer!Harry",
                fandom: 'Harry-Potter',
                favs: 110,
                follows: 167,
                genreA: 'Family',
    ...
    it('should handle strange fandom names', () => {
        expect(
            parser.parseSearchPage(
                'https://www.fanfiction.net/j/0/3/0/',
                [
        ...
        ).to.eql([
            {
                ...
                completed: false,
                description:
                    'this boy ichika Orimura was believed to have died in a kidnapping incident but he didn\'t but in exchange for staying with the living he was subjected to the horrors of being a child solider and fighting in a war This is an alternate version of a story made by "Azure Dragon of the East" this will be further explained in what would be the first chapter of the story',
    ...
    it('should parse a search result with boldness', () => {
        expect(
            parser.parseSearchPage(
                'https://www.fanfiction.net/search/?keywords=harry&ready=1&type=story',
                [
...
describe('parser parseUserPage', () => {
    it('should handle the basics', () => {
        expect(
            parser.parseUserPage(
                'https://www.fanfiction.net/u/5503799/Paersephone',
                [
                    `<span class="bff-span bff-success">up to date</span><a class="stitle" href="/s/13474353/1/Drabbles-OS-and-passing-thoughts"><img class="lazy cimage " style="clear: left; float: left; margin-right: 3px; padding: 2px; border: 1px solid rgb(204, 204, 204); border-radius: 2px; display: block;" src="/image/5920357/75/" data-original="/image/5920357/75/" width="50" height="66">Drabbles, OS, and passing thoughts</a> <a href="/s/13474353/2/Drabbles-OS-and-passing-thoughts"><span class="icon-chevron-right xicon-section-arrow"></span></a>
        ...
        ).to.eql([
            {
                ...
                completed: true,
                description:
                    "Multiple pairings and sometimes no pairings at all. I just want to have a kind of masterfic where everything can be stored, so it doesn't get too crowded on my profile yet. All HP-related, some AU, some canon. FIRST FOR NOW : Someone Who Cares. Neville doesn't feel so well after discovering who exactly was usurpating Moody's identity.",
            },
        ]);
    });
});
